...
\begin{figure}
\centering
<<letters, echo=FALSE, figure.width=8, figure.height=6, out.width='\\textwidth'>>=
letters_df$a <- ""
letters_df$b <- ""
letters_df$c <- ""
...
lineups$td <- with(lineups, paste(test_param, type, sep="."))

means <- ddply(lineups, .(scenario), summarize, means=mean(correct))
ggplot() + geom_vline(aes(xintercept=means), data=means, colour="grey50") +
  geom_point(aes(y=reorder(td, fit, mean),x=fit*100, colour=test_param),  size=4, data=type) + 
...
#  geom_line(aes(x=test_param,y=correct, group=param_value), data=lineups, alpha=0.5) + coord_flip() +
  ...
  theme_bw() + xlim(c(-17.5, 100)) + 
  scale_colour_brewer(palette="Set1", guide="none") +
  geom_text(aes(y=td, label=a), x=-8, data=letters_df, size=3) + 
  geom_text(aes(y=td, label=b), x=-6.5, data=letters_df, size=3) + 
  geom_text(aes(y=td, label=c), x=-5, data=letters_df, size=3) +
  geom_text(aes(y=td, label=test_param), x=-12.5, hjust=1, data=letters_df, size=3.5) + 
  ylab("") + xlab("Percent data picks") + 
  theme(axis.ticks.y=element_blank(), axis.text.y=element_blank())
@
\caption{\label{letterplot} Facetted dotplot of percent of data picks by type of charts; the three different panels show different scenarios. Designs are sorted according to model fit (large point). The labelling in each panel shows significant differences between designs: designs that do not have a letter in common, are significantly different on the 0.05 significance level. }
\end{figure}
